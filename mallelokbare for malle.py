...
class Mastermindgame:
    ...
    def check_guess(self):
        ...
        if correct_placement == 4 or self.guess_count >= number_attempts:
            player_name = tk.simpledialog.askstring("Highscore", "Skriv inn navnet ditt for highscore:")
            if player_name:
                post_highscore(self.guess_count, player_name)
            self.update_highscores()
        else:
            self.guess_count += 1
    ...
    def display_highscores(self):
        # Fjern eksisterende scorebord hvis det finnes
        if hasattr(self, "scoreboard"):
            self.scoreboard.destroy()

        # Opprett nytt scorebord og plasser det til venstre
        self.scoreboard = tk.Frame(self.root)
        self.scoreboard.pack(side=tk.LEFT, padx=10, pady=10)

        tk.Label(self.scoreboard, text="Top 5 Highscores", font=("Helvetica", 14, "bold")).pack()
...
def get_highscores():
    try:
        ...
        print(f"Feil ved henting av highscores: {e}")
        return []

def post_highscore(hs, player):
    try:
        data = {"id": GameID, "hs": hs, "player": player}
        resultat = req.post(URL, json=data)
        if resultat.status_code == 200:
            print("Highscore sendt inn!")
        else:
            print("Feil ved innsending av highscore.")
    except Exception as e:
        print(f"Feil ved innsending av highscore: {e}")
